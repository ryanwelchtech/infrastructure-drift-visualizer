...
import { getDriftSeverity, calculateDriftScore, formatDate } from '@/lib/utils';
...
function generateReport(resources: Resource[], summary: DriftSummary): string {
  const lines = [
    '# Infrastructure Drift Report',
    '',
    `**Generated:** ${formatDate(new Date())}`,
    `**Total Resources:** ${summary.total}`,
    `**Drift Score:** ${summary.score}%`,
    '',
    '## Summary',
    '',
    '| Status | Count |',
  ...
  resources.forEach(resource => {
    ...
    lines.push(`### ${statusIcon} ${resource.name} (${resource.type})`);
    lines.push(`- **ID:** ${resource.id}`);
    lines.push(`- **Status:** ${resource.status.toUpperCase()}`);
    lines.push(`- **Region:** ${resource.region}`);
    lines.push('');
  });

  lines.push('---');
  lines.push('*Generated by Infrastructure Drift Visualizer*');
...
export default function Home() {
  ...
  const handleExport = useCallback(() => {
    const report = generateReport(resources, summary);
    const blob = new Blob([report], { type: 'text/markdown' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `drift-report-${new Date().toISOString().split('T')[0]}.md`;
